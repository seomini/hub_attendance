...
//    let backgroundcolor = UIColor(red: 255/255, green: 252/255, blue: 247/255, alpha: 1.0)
    ...
    override func viewDidLoad() {
        ...
        findpwviewlbutton.setTitleColor(hubRedColor, for: .normal)
        ...
        // 사용자 아이디 입력 필드 설정
        useridTextField.placeholder = "아이디"
        useridTextField.borderStyle = .roundedRect
        useridTextField.frame = CGRect(x: paddingwidthSize, y: 300, width: widthSize, height: 40)
        useridTextField.center.x = screenSize.width / 2
        view.addSubview(useridTextField)
        
        // 사용자 이름 입력 필드 설정
        usernameTextField.placeholder = "이름"
        usernameTextField.borderStyle = .roundedRect
        usernameTextField.frame = CGRect(x: paddingwidthSize, y: 360, width: widthSize, height: 40)
        ...
        findidviewlbutton.addTarget(self, action: #selector(idfindButtonTapped), for: .touchUpInside)
